const Section = ({ title, children }) => (
  <div className="card p-4 px-4.5 mb-2.5">
    <h3 className="text-[12px] font-semibold text-dim uppercase tracking-wide mb-3.5">{title}</h3>
    {children}
  </div>
);
...
const BarRow = ({ label, pct, count, variant }) => {
  ...
  return (
    <div className="flex items-center gap-2.5 mb-2 text-[13px]">
      ...
      <div className="flex-1 h-2.5 bg-surface3 rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-[width] duration-500" style={{ width: `${pct}%`, background: fill }} />
      </div>
      <span className="w-10 text-[11px] text-muted flex-shrink-0 tabular-nums text-right">{count}회</span>
    </div>
  );
};
...
const StatsTab = ({ stats }) => {
  ...
  return (
    <div>
      ...
      <Section title={<><span aria-hidden="true">🔴</span> 핫넘버 / <span aria-hidden="true">🔵</span> 콜드넘버</>}>
        <div className="grid grid-cols-2 gap-1.5">
          {stats.lotto.hot5.map((x) => (
            <div key={x.number} className="flex justify-between items-center bg-surface2 px-3 py-2 rounded-lg border border-night">
              <span className="text-gold font-bold text-[14px]"><span aria-hidden="true">🔴</span> {x.number}번</span>
              <span className="text-muted text-[11px]">{x.count}회</span>
            </div>
          ))}
          {stats.lotto.cold5.map((x) => (
            <div key={x.number} className="flex justify-between items-center bg-surface2 px-3 py-2 rounded-lg border border-night">
              <span className="text-sky font-bold text-[14px]"><span aria-hidden="true">🔵</span> {x.number}번</span>
              <span className="text-muted text-[11px]">{x.count}회</span>
            </div>
          ))}
        </div>
      ...
      <Section title={<><span aria-hidden="true">📌</span> 연금복권 조별 출현</>}>
        {(() => {
      ...
      <Section title={<><span aria-hidden="true">📊</span> 연금복권 자리별 TOP 3</>}>
        {stats.pension.digit_stats.map((pos) => (
          <div key={pos.position} className="flex items-center gap-2.5 mb-2">
            <span className="w-12 text-right text-dim text-[12px] flex-shrink-0">{pos.position}</span>
            <span className="text-[13px] text-dim">
              {pos.top3.map((t) => (
                <span key={t.digit} className="mr-2">
                  <span className="text-sky font-bold">{t.digit}</span>
      ...
      <Section title={<><span aria-hidden="true">📐</span> 로또 당첨 합계 범위 (80%)</>}>
